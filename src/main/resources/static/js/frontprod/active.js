...
$('.qty-btn').on('click', function (e) {
    ...
    if ($button.hasClass('inc') && oldValue < max) {
        newVal = oldValue + 1;
    } 
    else if ($button.hasClass('dec') && oldValue > min) {
        newVal = oldValue - 1;
    }
...
function updateButtons() {
	...
	// 		$('#price-amount').val( '價格: ' + 'NT' + ui.values[ 0 ] + ' - NT' + ui.values[ 1 ] );
	// 	}
	// });
	// $('#price-amount').val( '價格: ' + 'NT' + $('#price-range').slider( 'values', 0 ) +
	// 	' - NT$' + $('#price-range').slider('values', 1 ) ); 
...
=============================================*/
    ...
    // 假設用 localStorage 控制登入狀態
    // 你可以依你的系統改成 cookie、session、token 判斷
    ...
    // 預設：登入按鈕顯示、基本資料隱藏
    if (isLoggedIn) {
        $loginItem.hide();       // 隱藏登入/註冊
        $profileItem.show();     // 顯示基本資料
    } else {
        $loginItem.show();
        $profileItem.hide();
    }
...
/*=====  End of login status control  ======*/
    ...
    const items = document.querySelectorAll(".shop-product-wrap .col-lg-4");
    const pageButtons = document.querySelectorAll(".pagination-wrapper ul li a");
    ...
    function showPage(page) {
        ...
        // 切換 active 樣式
        document.querySelectorAll(".pagination-wrapper ul li").forEach(li => li.classList.remove("active"));
        const currentLi = document.querySelector(`.pagination-wrapper ul li:nth-child(${page})`);
    ...
    pageButtons.forEach(button => {
        button.addEventListener("click", function (e) {
            ...
            const page = parseInt(this.textContent) || null;

            // 處理下一頁箭頭
            if (!page && this.querySelector(".fa-angle-right")) {
                const current = document.querySelector(".pagination-wrapper ul li.active");
	...
	document.querySelectorAll('.filter-btn[data-type="price"]').forEach(btn => {
	    btn.addEventListener('click', () => {
	        const min = btn.dataset.min;
	        const max = btn.dataset.max;
	        window.location.href = `/price?minPrice=${min}&maxPrice=${max}`;
	    });
	});
